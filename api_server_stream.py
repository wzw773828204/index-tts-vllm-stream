...
@app.post("/index-tts/v1/tts_stream", responses={
    200: {"content": {"application/octet-stream": {}}},
    500: {"content": {"application/json": {}}}
})
async def tts_api_stream(request: Request):
    async def generate():
        global tts
        async for raw in tts.infer_with_ref_audio_embed_stream(reference_id, text):
    ...
    try:
        ...
        # return Response(content=wav_bytes, media_type="audio/wav")
        return StreamingResponse(generate(), headers={"Content-Disposition": f"attachment; filename=audio.wav"}, media_type="audio/wav")
